...
public class Model {
    ...
    public static final int MAX_ROBOT_ENERGY = 1 * Panel.APS * 60; // Panel.APS is set to 1
    ...
    public Model() {
        ...
        for (int i = 0; i < NUM_ROBOTS; i++) {
            ...
            do {
                ...
                row = RAND.nextInt(1, GRID.length);
                col = RAND.nextInt(1, GRID[row].length);

            } while (robotCells.contains(GRID[row][col]));

            robotCells.add(GRID[row][col]);

            robots.add(new Robot(i, MAX_ROBOT_ENERGY, GRID[row][col]));
    ...
    public String toString() {
        ...
        String robotEnergies = " Energy: ";

        for (int i = 0; i < robots.size(); i++) {
            robotEnergies += String.format("%3d ", robots.get(i).energy);
        }

        return String.format(
                "|%s|  Served: %3d |  Collisions: %3d |  Evaluation: %3d |\n",
                robotEnergies, numServedClients, numCollisions, evaluation);
    }
