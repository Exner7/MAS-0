...
public class RouteGraph {
    ...
    public ArrayList<Integer> search(RouteProblem problem) {
        ...
        while (true) {
            ...
            // MAS2 : Positive cost addition...
            if (node.pathCost < bestNode.pathCost ||
                    0 < node.pathCost && problem.goalTest(node.state)) {
                bestNode = node;
            }
    ...
    private ArrayList<Integer> solution(
            RouteProblem problem, RouteNode bestNode) {

        ArrayList<Integer> solution = new ArrayList<>();

        RouteNode node = bestNode;
        ...
        if (solution.size() <= 0) {
            plan.add(0); // zero cost
            plan.add(9); // wait action
            return plan;
        }
